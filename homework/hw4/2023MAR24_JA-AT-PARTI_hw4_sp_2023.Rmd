...
pacman::p_load(bestglm, glmnet, leaps, car, tidyverse, pROC, caret, ROCR) # add the packages needed
```
...
&=Prob((HD=0|SBP=118), (HD=0|SBP=140), (HD=0|SBP=160), (HD=1|SBP=165), (HD=1|SBP=215)) \\
&=Prob(HD=0|SBP=118) \times Prob(HD=0|SBP=140) \times Prob(HD=0|SBP=160) \times Prob(HD=1|SBP=165) \times Prob(HD=1|SBP=215) ) \\
...
```{r}
summary(fit0, results = T)
```


_The MLE are obtained numerically by maximizing the log likelihood function are above to estimate the unknown $\beta 's given the data$_
...
_``SEX`` is the single cariable that is the most important to add given the lowest AIC for that model_
```{r}
...
_The residual deviance of `fit2`, which is the smaller model, may not be consistently smaller than `fit1`. This is because the residual deviance is influenced by the the features in the proposed model compared to the saturated model, and while fit2 may exhibit the best feature to add to the model sequentially, it may not necessarily be the best fit for the data overall._
...
Anova(fit2)
```
_the pvalue for SEX is 3.8e-11_

```{r}
confint.default(fit2)
```
_The confidence intervals do not contain 0 for any of the coefficients, so we know that the true betas for the variables are not 0._
...
```
_the pvalue for SEX is 3.8e-11, which is the same as the type II test_


```{r}
chi.sq <- 1469-1374
pchisq(chi.sq, 1, lower.tail=F)
```


_the pvalue for this likelihood ratio test is 1.98e-22, which is still significant, but different as it's based of the entire model comparison of deviance residuals_
...
_DBP has the highest p-value, so we will remove it. _
```{r}
fit3.1 = update(fit3, .~. - DBP)
summary(fit3.1)
```

_FRW has the highest p-value, so we will remove it. _
```{r}
...
_CIG has the highest p-value, so we will remove it. _
```{r}
...
```{r}
# Liz = 
final.model.Liz = predict(final.model, hd_data.f, type="response")
```
...
_The AUC of `fit2` is generally larger than that of `fit1`, even with potential crossover in lower left of he plot._
...
confusionMatrix(data = as.factor(fit1.5),
                reference = hd_data.f$HD,
                positive = levels(hd_data.f$HD)[2])
```

_Here we see the Positive Prediction Values and Negative Prediction Values for `fit1` are 0.45 and 0.783, respectively. They are calculated manually and confirmed with the confusionMatrix function._
...
confusionMatrix(data = as.factor(fit2.5),
                reference = hd_data.f$HD,
                positive = levels(hd_data.f$HD)[2])
```
_Here we see the Positive Prediction Values and Negative Prediction Values for `fit2` are 0.472 and 0.786, respectively. They are calculated manually and confirmed with the confusionMatrix function._
...
_I would chose fit2 because it has a lower false positive rate than fit1 at lower thresholds._
...
If the cost of misclassifying "1" to "0" is 10 times of that of misclassifying "0" to "1",
then the Bayes rule is thresholding over the $$\hat P(Y=1 \vert x) > \frac{0.1}{(1+0.1)}=0.0909$$
or $$logit > \log(\frac{0.0909}{0.909})=-2.3$$ 

The linear boundary is $$\hat{HD} = 1 \text{ if }  0 < 0.06153 \text{ AGE } + 0.91127 \text{ SEX-MALE } + 0.01597 \text{ SBP } + 0.00449 \text{ CHOL } + 0.006043 \text{ FRW } + 0.01228 \text{ CIG } - 6.93$$


ii. What is your estimated weighted misclassification error for this given risk ratio?
```{r}
fit.final.pred.bayes <- as.factor(ifelse(final.model$fitted > .0909, "1", "0"))
MCE.bayes <- (10*sum(fit.final.pred.bayes[hd_data.f$HD == "1"] != "1")
              + sum(fit.final.pred.bayes[hd_data.f$HD == "0"] != "0"))/length(hd_data.f$HD)
MCE.bayes
```

_The weighted misclassification error for this given risk ratio is 0.714_
...
pred1 <- prediction(fit1$fitted,hd_data.f$HD)
perf1 <- performance(pred1,"ppv", "npv")
plot(fit1.roc$thresholds,
     perf1@y.values[[1]], col="red", lwd=3, type="l",
     ylim=c(0,1),
     xlab="Threshold on prob",
     ylab="prediction values")
lines(fit1.roc$thresholds, perf1@x.values[[1]], col="blue", lwd=3)
legend("bottomright",
       c("Positive prediction values",
...
```{r}
pred2 <- prediction(fit2$fitted,hd_data.f$HD)
perf2 <- performance(pred2,"ppv", "npv")
plot(fit2.roc$thresholds,
     perf2@y.values[[1]], col="red", lwd=3, type="l",
     ylim=c(0,1),
     xlab="Threshold on prob",
     ylab="prediction values")
lines(fit2.roc$thresholds, perf2@x.values[[1]], col="blue", lwd=3)
legend("bottomright",
       c("Positive prediction values",
         "Negative prediction values"),
       col=c("red", "blue", "green"),
...
If the cost of misclassifying "1" to "0" is 1 times of that of misclassifying "0" to "1",then the Bayes rule is thresholding over the $$\hat P(Y=1 \vert x) > \frac{1}{(1+1)}=0.5$$ or $$logit > \log(\frac{05}{0.5})=0$$

The linear boundary is $$\hat{HD} = 1 \text{ if }  0 < 0.06153 \text{ AGE } + 0.91127 \text{ SEX-MALE } + 0.01597 \text{ SBP } + 0.00449 \text{ CHOL } + 0.006043 \text{ FRW } + 0.01228 \text{ CIG } - 9.22786$$

```{r}
fit.final.pred.bayes.1 <- as.factor(ifelse(final.model$fitted > .5, "1", "0"))
MCE.bayes.1 <- (10*sum(fit.final.pred.bayes.1[hd_data.f$HD == "1"] != "1")
              + sum(fit.final.pred.bayes.1[hd_data.f$HD == "0"] != "0"))/length(hd_data.f$HD)
MCE.bayes.1
```

_the weighted misclassification error is 2.05_
