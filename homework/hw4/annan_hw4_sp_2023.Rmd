...
pacman::p_load(bestglm, glmnet, leaps, car, tidyverse, pROC, caret, xtable, ROCR) # add the packages needed
```
...
hd_data.liz <- data.frame("AGE"=50, "SEX"="FEMALE", "SBP"=110, "DBP"=80, "CHOL"=180, "FRW"=105, "CIG"=0)
...
set.seed(28)
indx <- sample(1393, 5)
fram_sub<- hd_data.f[indx, c("HD","SBP")] 
...
copy MLE explanation section from lecture

```{r results=TRUE}
fit0 <- glm(HD~SBP, hd_data.f, family=binomial(logit)) 
...
```{r}
fit0.predict <- predict(fit0, hd_data.liz, type="response")
fit0.predict
fit01.predict <- predict(fit0, hd_data.new, type="response")
fit01.predict
```
...
Xy_design <- model.matrix(HD ~.+0, hd_data.f) # Attach y as the last column.
...
fit.final <- glm(HD~AGE+SEX+SBP+CHOL+FRW, family=binomial, data=hd_data.f)
```
...
```{r}
fitfinal.predict <- predict(fit.final, hd_data.liz, type="response")
fitfinal.predict
```
...
plot(1-fit1.roc$specificities,
     ...
     ylab="Sensitivity")
legend("bottomright",
       c(paste0("fitLasso AUC=", round(final.fit.lasso2.roc$auc, 2))),
       col=c("red"),
       lty=1)
```
...
fit2.pred.5 <- as.factor(fit2.pred.5)
confusionMatrix(data = fit2.pred.5,
                reference = hd_data.f$HD,
                positive = levels(hd_data.f$HD)[2])
```
...
If the cost of misclassifying "1" to "0" is 1 times of that of misclassifying "0" to "1",
then the Bayes rule is thresholding over the $$\hat P(Y=1 \vert x) > \frac{1}{(1+1)}=0.5$$
or $$logit > \log(\frac{05}{0.5})=0$$ 
...
```{r}
fit.final.pred.bayes.1 <- as.factor(ifelse(fit.final$fitted > .5, "1", "0"))
