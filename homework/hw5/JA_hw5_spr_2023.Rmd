...
pacman::p_load(randomForest, tree, ISLR, rpart, rattle, pROC, partykit, ggplot2, glmnet, leaps, dplyr, keras, neuralnet, imager, ranger, lubridate, tm, RColorBrewer, wordcloud, SnowballC)
```
...
# tf$constant("Hello")
# 
# # Sys.setenv(RETICULATE_PYTHON = "/Users/jeneaadams/Library/r-miniconda-arm64/envs/mlp/bin/python")
# 
# 
...
weekdays = weekdays(yelp_data$date) # get weekdays for each review  
months <- months(yelp_data$date)   # get months 


yelp_data %>%
  ggplot(aes(x = months, y = stars)) +
...
# inspect(dtm1[1,]) 
# 
# threshold <- .05*length(mycorpus_clean)   # 5% of the total documents 
# words.5 <- findFreqTerms(dtm1, lowfreq=threshold)  # words appearing at least among 5% of the documents
# length(words.5)
# 
# dtm.5<- DocumentTermMatrix(mycorpus_clean, control = list(dictionary = words.5))  
# dim(as.matrix(dtm.5))
# colnames(dtm.5)[40:50]


```
 
Cleaner way ot do the above 
```{r}
mycorpus1  <- VCorpus(VectorSource(data1.text))
...
control_list <- list( tolower = TRUE, 
                      ...
                      stemming = TRUE)
# dtm with all terms:
dtm.5.long  <- DocumentTermMatrix(mycorpus1, control = control_list)
#inspect(dtm.10.long)

# kick out rare words 
dtm.5<- removeSparseTerms(dtm.5.long, 1-.005)
...
_This matrix records the frequency of each of the top words (columns) across each of the documents (rows)._

```{r}
dim(as.matrix(dtm.5))
# 
# inspect(dtm.5[100,405]) ## need to find this 
```


b) What is the sparsity of the dtm obtained here? What does that mean?

_97% sparsity means that there is 97% sparse entries in the matrix_
...
```{r}
yelp_data$rating = ifelse(yelp_data$stars == 5 | yelp_data$stars == 4, 1, 0)

# data2 = cbind(dtm.5, yelp_data$rating)
data2 = data.frame(yelp_data$rating, as.matrix(dtm.5))
...
```{r}
data2 = data2 %>%
  rename(rating = yelp_data.rating)
```
...
train.index = sample(n, n1)
no.train.index = (which(!seq(1:n) %in% train.index))


test.index = sample(no.train.index, n2)

val.index = which(! no.train.index %in% test.index)


data2.train = data2[train.index,]
data2.test = data2[test.index,]
data2.val = data2[val.index,]
...
fit.lasso = cv.glmnet(X1, Y, alpha = 1, family = "binomial")
...
_The tuning parameters that are chosen at the end are ... _

ii. Feed the output from Lasso above, get a logistic regression and call this fit.glm
```{r}
# glm.input <- as.formula(paste("rating", "~", paste(coef.1se[-1],collapse = "+"))) # prepare the formulae

glm.in = select(data2.train, all_of(c("rating", lasso.words)))


result.glm <- glm(glm.in, family=binomial, data2.train ) 
summary(result.glm)
...
```{r}
pos.coefs = result.glm.coef[which(result.glm.coef > 0)]
pos.coefs = pos.coefs[-1] # remove the intercept

pos.coefs.sort = sort(pos.coefs, decreasing = T)
pos.coefs.sort[1:2]

good.word = names(pos.coefs.sort)
```

_thorough and delight are the 2 leading words. These positive coefficients means that they are positively associated with good ratings by reviewers. This probably also means that these are more positive words_
...
wordcloud(good.word[1:100], pos.coefs.sort[1:100],  # make a word cloud
          colors=cor.special, ordered.colors=F)
```
_The larger words have a higher frequency and are more likely to be associated with positive reviews in this case._


c) Repeat a) and b) above for the bag of negative words.
```{r}
neg.coefs = result.glm.coef[which(result.glm.coef < 0)]
neg.coefs = neg.coefs[-1] # remove the intercept

neg.coefs.sort = sort(-neg.coefs, decreasing = T) 
neg.coefs.sort[1:2]

bad.word = names(neg.coefs.sort)

cor.special <- brewer.pal(6,"Dark2")  # set up a pretty color scheme
wordcloud(bad.word[1:100], neg.coefs.sort[1:100],  # make a word cloud
          colors=cor.special, ordered.colors=F)
```

_"worse" and "rude" are word most associated with negative reviews.  _

d) Summarize the findings. 

iii. What are the major differences among the two methods used so far: Lasso and glm

_the major differences between LASSO and the GLM is that LASSO uses a regularization technique to select important features, whereas in our GLM minimizes all of the coefficients._

iv.  Using majority votes find the testing errors
	a) From `fit.lasso`
	
```{r}
predict.lasso.p <- predict(fit.lasso, as.matrix(data2.test[, -1]), type = "response", s = "lambda.1se") # lasso estimates of probabilities
predict.lasso <- predict(fit.lasso, as.matrix(data2.test[, -1]), type = "class", s="lambda.1se") #vote labels
  # output majority vote labels

# LASSO testing errors
mean(data2.test$rating != predict.lasso) # 0.1362

# ROC curve for LASSO estimates

pROC::roc(data2.test$rating, predict.lasso.p, plot=TRUE) #AUC = 0.9284
```
...
testerror.glm   # mis classification error is 0.2164
...
_the LASSO testing error is smaller 0.1362 compared to the GLM's 0.2164_
...
# set_random_seed(10)
# 
# p <- dim(data2.train)[2] # number of input variables
# 
# model <- keras_model_sequential() %>%
#   layer_dense(units = 20, activation = "relu", input_shape = c(p)) %>% 
#   # 1 layer with 20 neurons. default activation is relu
...
_RandonForest takes the average of many bootstrap trees to move away from overfitting and to generate many equations for different groups of variables._
...
```{r}
set.seed(1)
fit.rf.5 = randomForest(rating~., data2.train, mtry=5, ntree=100) #1 by default, the minsize = 5 in regression tree. 
...
```{r}
fit.rf.pred <- predict(fit.rf, data2.train)
MSE_Train <- mean((data2.train$rating - fit.rf.pred)^2)     # training error
```
...
head(fit.rf$oob.times) # how many times each obs'n belong to OOB. We expect to see 1/e=1/3 (.37)

predicted_oob <- fit.rf$predicted 

mse_oob <- mean((data2.train$rating - predicted_oob)^2)  
...
```{r}
# fit.rf <- randomForest(rating~., data2.train, mtry=5, ntree=500)    # change ntree
# plot(fit.rf, col="red", pch=16, type="p", 
#      main="default plot, ")
...
```{r}
iq1$Imagazine = as.factor(iq1$Imagazine)
iq1$Inewspaper = as.factor(iq1$Inewspaper)
iq1$Ilibrary = as.factor(iq1$Ilibrary)
iq1$Race = as.factor(iq1$Race)
iq1$Gender = as.factor(iq1$Gender)
```

+ Make log transformation for Income and take the original Income out
```{r}
iq2 = iq1 %>%
  mutate(log_fam_income = log(FamilyIncome78), 
         log_05_income = log(Income2005)) %>%
  select(-FamilyIncome78, -Income2005)
```

+ Take the last person out of the dataset and label it as **Michelle**. 

```{r}
Michelle = iq2[nrow(iq2),]
...
test.index.iq2 = sample(n, n*.2)

iq.test <- iq2[test.index.iq2, ]
iq.nottest <- iq2[-test.index.iq2,]

n2 <- nrow(iq.nottest)
valid.index.iq <- sample(n2, n*.1)
iq.val <- iq.nottest[valid.index.iq,]
