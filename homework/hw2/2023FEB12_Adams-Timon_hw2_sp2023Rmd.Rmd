...
```{r  message=FALSE, warning=FALSE, eval = TRUE, echo = F}
...
_We will use 2 PCs for the centered and scaled and 4 for the centered but not scaled_
...
_We should not scale the because scaling reduces the separation of clusters_
```{r message=FALSE, warning=FALSE, echo=FALSE, fig.height=6, fig.width=12, results='asis'}
# pc scores
...
_We choose 3 clusters after looking at the elbow plot_
...
_Clustering is able to separate basal fairly well; however, it doesn't do as well for Luminal A, Lumnial B and HER2_
...
_PCA yields better clustering than applying kmeans to the original data. PCA helps because it is able to capture the variance of the data._
    
```{r, echo=FALSE, fig.height=6, fig.width=12, results='asis'}
# elbow methodfor optimal number of clusters
...
```{r message=FALSE, warning=FALSE, fig.height=6, fig.width=12, results='asis', echo=FALSE}
...
_It is difficult to tell which subtype it is since Luminal A, Lumnial B and HER2 are tightly clustered together, but we can definitely rule ot basal. It is Probably LiminalB or HER2 _
...
kable(data.frame(summary(Auto$mpg)))
...
kable(data.frame(summary(Auto$cylinders)))
...
kable(data.frame(summary(Auto$displacement)))
...
kable(data.frame(summary(Auto$horsepower)))
...
kable(data.frame(summary(Auto$weight)))
...
kable(data.frame(summary(Auto$acceleration)))
...
select(Auto, colnames(Auto)[!(colnames(Auto) %in% c("name", "origin"))]) %>%
...
In this final model, we consider both cylinders and origin as categorical variables. We remove acceleration and we assume interaction between year-horsepower and origin-displacemnt
...
```{r message=FALSE, warning=FALSE, echo=F, results='asis', fig.height=8, fig.width=12}
ggplot(Auto, aes(x = displacement, y = mpg, color = origin)) +
  geom_point() +
  geom_smooth(method = "lm" , se =F) +
  facet_wrap(~ origin) +
  labs(title="mpg and displacement by cylinders")
```

```{r message=FALSE, warning=FALSE, echo=FALSE, results='asis'}
# everything
fit6 <- lm(mpg ~ cylinders + displacement + horsepower + weight + acceleration + year + origin, data = Auto) 

# everything with cylinders as factor
fit7 <- lm(mpg ~ factor(cylinders) + displacement + horsepower + weight + acceleration + year + origin, data = Auto) 

# year horse power interactions
fit8<- lm(mpg ~ cylinders + displacement + year*horsepower + weight + acceleration + origin, data = Auto) 

# year horse power interactions and cylinders as factor
fit9<- lm(mpg ~ factor(cylinders) + displacement + year*horsepower + weight + acceleration + origin, data = Auto) 

# year horse power interactions and cylinders as factor and remove displacement
fit10<- lm(mpg ~ factor(cylinders) + year*horsepower + weight + acceleration + origin + displacement, data = Auto)

# year horse power interactions and cylinders as factor and remove acceleration
fit11<- lm(mpg ~ factor(cylinders) + year*horsepower + weight + origin + displacement, data = Auto)

stargazer(fit6, fit7, fit8, fit9, fit10, fit11,  type = output_format, ci=TRUE, ci.level = .95,
                     keep.stat = c("n", "rsq", "sigma2", "ser"))

```


```{r message=FALSE, warning=FALSE, echo=FALSE, results='asis', fig.height=6, fig.width=10}
par(mfrow=c(1,2))
plot(fit11, 1)
plot(fit11, 2)
```
...
```{r message=FALSE, warning=FALSE, echo=FALSE, results='asis'}
stargazer(fit11,  type = output_format, ci=TRUE, ci.level = .95,
                     keep.stat = c("n", "rsq", "sigma2", "ser"))
```
...
predict(fit11, newcar, interval = "predict", se.fit = TRUE)
```
