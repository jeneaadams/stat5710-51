...
```{r read data}
survey_data <- read.csv("data/Survey_results_final.csv", header=T, stringsAsFactors = FALSE)
names(survey_data)
```
```{r rename and subset data}
survey_data_subset <- rename(survey_data,
                             age = Answer.Age,
  ...
  select(. ,c("age", "gender", "education", "income", "sirius", "wharton", "worktime"))

head(survey_data_subset)

str(survey_data_subset)
summary(survey_data_subset)
```
...
* Within the `age` variable, 4 people did not respond and 2 people selected ages that did not make sense in the context of the survey (4 and 223). Since these were likely errors on the part of the participant, we decided to fully remove their surveys

```{r examine age, echo=FALSE}
# coerce to numeric 
age <- survey_data_subset$age %>% as.numeric(.)
summary(age)
head(age[order(age)])
tail(age[order(age)])

age <- age[(!is.na(age) & !(age %in% c(4,223)))]
summary(age)
...
```{r examine gender}
# coerce to factor 
survey_data_subset$gender %>% as.factor(.) %>% summary(.)
...
```{r examine education}
education <- survey_data_subset$education %>% as.factor(.)
summary(education)

# 19 people selected the invalid response "select one" will chage these responses to 'other'
education <- recode_factor(education, 'select one' = "Other")
summary(education)
```
...
```{r examine income}
# coerce to factor 
survey_data_subset$income %>% as.factor(.) %>% summary(.)
...
```{r examine worktime}
survey_data_subset$worktime %>% summary(.)
