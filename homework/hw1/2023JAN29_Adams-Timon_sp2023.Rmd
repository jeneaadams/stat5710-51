...
pacman::p_load(ISLR, readxl, tidyverse, magrittr, dplyr, ggplot2,readxl,reshape2, ggpmisc, knitr, ggpubr)
```
...
survey.sub.pruned$income <- as.factor(survey.sub.pruned$income)

survey.sub.pruned$education <- as.factor(survey.sub.pruned$education)

survey.sub.pruned$gender <- as.factor(survey.sub.pruned$gender)

# discard values based on age
...
- education 
```{r}
kable((summary(survey.sub.pruned$education)/length(survey.sub.pruned$education))*100,
      caption = "Survey Demographics by education")
```
- Income
```{r}
kable((summary(survey.sub.pruned$income)/length(survey.sub.pruned$income))*100,
      caption = "Survey Demographics by education")
```
- worktime
...
survey.sub.pruned$income <- factor(survey.sub.pruned$income,
                                     levels = c("Less than $15,000",
                                                ...
                                                "$75,000 - $150,000",
                                                "Above $150,000"))
((summary(survey.sub.pruned$income)/length(survey.sub.pruned$income))*100)
# plot looking at income total
ggplot(survey.sub.pruned, aes(x=income, fill=income))+
...
ggplot(survey.sub.pruned, aes(x=income, group=gender))+
...
ggplot(survey.sub.pruned, aes(x=income, y=age, fill=income)) +
  geom_boxplot() +
  scale_fill_brewer(palette="Reds")
...
ggplot(survey.sub.pruned, aes(x=education, fill=education))+
  geom_bar(aes(y = (..count..)/sum(..count..)))  +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..)),
                y = ((..count..)/sum(..count..))), stat= "count", vjust = -.5) +
...
ggplot(survey.sub.pruned, aes(x=education, group=gender))+
  geom_bar(aes(y = ..prop.., fill =factor(..x..)), stat="count") +
  geom_text(aes(label = scales::percent(..prop..),
                y= ..prop..), stat= "count", vjust = -.5) +
  labs(title = "Survey Highest Degree", y = "Percent", x ='Highest Degree') +
...
ggplot(survey.sub.pruned, aes(x=education, y=age, fill=education)) +
  geom_boxplot() +
  scale_fill_brewer(palette="Reds")
```
...
ggplot(survey.sub.pruned, aes(x=sirius, fill=sirius))+
  geom_bar(aes(y = (..count..)/sum(..count..)))  +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..)),
                y = ((..count..)/sum(..count..))), stat= "count", vjust = -.5) +
  labs(title = "Self reported listeners to SiriusXM", y = "Percent", x ='Answer') +
  theme_minimal() +
  scale_fill_brewer(palette="Greens")+
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
...
survey.sub.pruned[survey.sub.pruned$sirius =='Yes',] %>% 
  ggplot(., aes(x=gender, fill = gender)) +
  geom_bar(aes(y = (..count..)/sum(..count..)))  +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..)),
                y = ((..count..)/sum(..count..))), stat= "count", vjust = -.5) +
  labs(title = "People who listen to sirius by gender", y = "Percent", x ='Answer') +
  theme_minimal() +
  scale_fill_brewer(palette="Reds")+
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
...
ggplot(survey.sub.pruned, aes(x=wharton, fill=wharton))+
  geom_bar(aes(y = (..count..)/sum(..count..)))  +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..)),
                y = ((..count..)/sum(..count..))), stat= "count", vjust = -.5) +
...
survey.sub.pruned[survey.sub.pruned$wharton =='Yes',] %>% 
  ggplot(., aes(x=gender, fill = gender)) +
  geom_bar(aes(y = (..count..)/sum(..count..)))  +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..)),
                y = ((..count..)/sum(..count..))), stat= "count", vjust = -.5) +
...
ggplot(survey.sub.pruned, aes(x=gender, fill=gender))+
  geom_bar(aes(y = (..count..)/sum(..count..)))  +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..)),
                y = ((..count..)/sum(..count..))), stat= "count", vjust = -.5) +
  labs(title = "Participants by Gender", y = "Percent", x ='Gender') +
...
1. Method proposed to estimate the audience size.
  - Since we know that most participants have some sort of college experience, we could poll audiences within and adjecnt tot hose speces with a qualtrics survey senct across the country. The incentive to participate would be cash prizes raffled off to the top N participants and/or one year of subscription service to Sirus XM radio. 
...
Is there evidence that more males are in science-related fields vs `Non-S&E`? --> There are more people overall in nons&e fields, in which there are more women. However, there are more men in the S&E fields from these plots. 
