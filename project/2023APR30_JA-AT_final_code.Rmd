...
Gini indexes have changed over time. Here we select the top and bottom 5 census tracts based on theer mean Gini coefficient from 2011 to 2019.  
...
Next we perform model selection on models to predict diabetes and COPD prevelance since they appear to have a strong positive correlation to gini index. 
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
fit.forward.diab = regsubsets(final_data$DIABETES_CrudePrev ~., cbind(final_data[-1][features], final_data$DIABETES_CrudePrev), nvmax = 60, method = "forward")
# summary(fit.forward.diab)

fit.forward.diab$rss
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
fit.backward.diab = regsubsets(final_data$DIABETES_CrudePrev ~., cbind(final_data[-1][features], final_data$DIABETES_CrudePrev), nvmax = 60, method = "backward")
# summary(fit.backward.diab)

which.min(fit.backward.diab$rss)
```
...
plot(lasso_models$DIABETES_CrudePrev, main = "Diabetes\n")
print(summary(models_min$DIABETES_CrudePrev))
...
## Reduced Linear Model Evaluation

### Diabetes

```{r}
# # diab.full = lm(data = final_data[-1], DIABETES_CrudePrev ~.)
# diab.full = lm(final_data$DIABETES_CrudePrev ~., cbind(final_data[-1][features], final_data$DIABETES_CrudePrev))
# 
# anova(diab.full, lasso_models$DIABETES_CrudePrev)
```
