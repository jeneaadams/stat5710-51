...
pacman::p_load(sf, readxl, tidyverse, ggplot2, ggpmisc, ggpubr, censusxy,
               ...
               skimr, GGaly, reshape2, glmnet, caret, viridis, pheatmap) 
```
...
Here is the break down of variable namea:
...
#### COPD 
```{r}
# fit.forward.copd = regsubsets(COPD_CrudePrev ~., final_data[-1][features], nvmax = 60, method = "forward")
...
```{r}
# fit.backward.diab = regsubsets(DIABETES_CrudePrev ~., final_data[-1][features], nvmax = 60, method = "backward")
...
#### COPD 
```{r}
# fit.backward.copd = regsubsets(COPD_CrudePrev ~., final_data[-1][features], nvmax = 60, method = "backward")
# summary(fit.backward.copd)

fit.backward.diab = regsubsets(final_data$COPD_CrudePrev ~., cbind(final_data[-1][features], final_data$COPD_CrudePrev), nvmax = 60, method = "forward")
summary(fit.backward.copd)
```

```{r}
coef(fit.backward.copd, 19)
```
