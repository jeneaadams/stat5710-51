...
pacman::p_load(censusxy, sf, readxl, tidyverse, ggplot2, ggpmisc, ggpubr, censusxy, tigris, ndi) 
```
...
## tidying up 
```{r}
merge3$TractFIPS = as.character(merge3$TractFIPS)
```
...
merge3$N_VERYHIGH = as.character(merge3$N_VERYHIGH)

merge3 %>%
  ggplot(aes(x = avgPCT_HPSS, y = BPHIGH_CrudePrev, size = avgPCT_POVERTY, color = N_VERYHIGH)) + 
  ...
  labs(x = "Average % High Produce Supply Stores", y = "Prevalenace of High Blood Pressure", title = "Integrated view of health disparities, poverty, and environmental factors" ) +
...
# geolocating 
Geolocating addresses to census tracts?
```{r}
...
```{r}
merge3.1 %>%
  ggplot(aes(x = sumTOTAL_UNITS, y = CANCER_CrudePrev, size = avgPCT_POVERTY, color = N_VERYHIGH)) + 
  geom_point(alpha = 0.4) +
  geom_smooth(method = "lm", se = F, show.legend = F) +
  # stat_poly_eq(use_label(c("eq", "R2"))) + 
  # stat_regline_equation(label.y = 60, aes(label = ..eq.label..), show.legend = F) +
  # stat_regline_equation(label.y = 55, aes(label = ..rr.label..), show.legend = F) +
  labs(x = "Total Affordable Housing Units", y = "Prevalenace of Cancer", title = "Integrated view of health disparities, poverty, and environmental factors" ) +
  scale_color_discrete(labels=c('No', 'Yes')) +
  guides(size=guide_legend(title="Average % Poverty"), 
         color=guide_legend(title="High Heat-Related Illness")) +
  theme_bw() + 
...
Testing on looking at property tax delinquencies 


## walkscores from Annan
```{r}
...
merge3.2 %>%
  ...
  scale_color_gradient(low = "lightpink", high = "darkblue", na.value = NA) +
...
#trying to see if i can conditionally change the fill on the points
merge3.2 %>%
  ggplot(aes(x = sumTOTAL_UNITS, 
             y = transit_score, 
             color = CANCER_CrudePrev, 
             # shape = N_VERYHIGH, 
             fill = ifelse(N_VERYHIGH == 1, NA, N_VERYHIGH))) + 
